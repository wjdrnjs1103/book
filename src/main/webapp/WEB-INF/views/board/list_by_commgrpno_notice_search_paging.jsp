...
<script type="text/javascript">
  ...
  function b_c_btn(commgrpno) {
    ...
    if ('${sessionScope.id}' == '') {  // 로그인이 안되어 있다면
      ...
      // 공지사항 등록 : 관리자만 가능(memberVO.mname=관리자, 로그인된 ID = admin)
  ...
  <%-- 공지사항 등록 --%>
  function board_create_ajax(commgrpno) {
    var f = $('#frm_login');
    var commgrpno = $('#commgrpno', f).val();  
...
<div class="py-5">
    ...
    <DIV class='container c_bottom'> 
        <DIV class='title_line_none'>
          ...
          <a href="./list_by_commgrpno_notice_search_paging.do?commgrpno=${param.commgrpno }&word=&now_page=1" class='title_link'>${commgrpVO.name}</a>
        </DIV>
